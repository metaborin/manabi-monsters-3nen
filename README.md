...
- 5教科エリア（国語・算数・理科・社会・英語）でクエストを遊べる（各教科に2つ以上のクエストがあり、国語・算数はさらにもう1つ）
- 問題に答えてコインと経験値を獲得できる
- クエストをクリアすると新しい仲間モンスター（全10体）が図鑑に増えていく
- 図鑑で仲間モンスターを確認できる
- 図鑑・結果画面・まなび島でモンスター画像を表示できる
- ホーム画面の教科カードとクエスト画面で、教科ごとの画像素材を表示できる
- ショップでアイテムを購入できる
- まなび島に仲間やアイテムを飾れる
- ホーム画面に島全体マップを表示し、施設アイコン（国語・算数・理科・社会・英語・ショップ・図鑑・まなび島・広場）を押して各画面へ移動できる（スマホ幅では専用マップと専用座標に切り替え、地図の下には従来のカード型メニューも残している）
- エリアや施設に入るとき、短い入場演出（到着メッセージ）が出る
- 「つぎのおすすめ」や各エリアの進行状況（つづき・クリア済みなど）が一目でわかる
- クエスト選択画面はステージ選択風で、状態バッジ（NEW/おすすめ/クリア済み）・ステージ番号プレート・報酬コインプレート・仲間モンスターの円フレームを画像素材で楽しく表示（画像が無い場合は絵文字などにフォールバック）
